...
Given a parameterized protocol with its local transition set $T$, the algorithm decides whether a livelock exists
...
We further independently demonstrate decidability of the livelock detection problem by proving that every livelock witness is bounded by a function of$|T|$, independent of K.
...
Farahat~\citep{farahat2012} for the same problem, where pseudolivelock and propagation are
...
We further show that the undecidability result established under the tiling formulation does not stem from the intrinsic behavior of the original model, but rather from an over-approximation introduced by that representation. Concretely, we prove that any livelock admits a witness whose size is bounded solely as a function of $|T|$, independently of the ring size $K$. This boundedness eliminates the source of undecidability and yields decidability.
...
$H^{e} = E^K$ on simple local cycles. That work is existential and does
...
and has own value equal to the written value $w'$ of any transition $t \in T$. Intuitively,
after $t$ fires and sets $x_r\!\leftarrow\!w'$, process $P_r$ cannot
fire again if the predecessor value stays at $v$, because no
...
\subsection{Intuition of the Livelock Witness}

We illustrate a graph-theoretic intuition to ease the reasong about our livelock witness. To this end, we leverage a symmetric $4$-coloring transitions set 
\[
T_c = \{ (v, v, (v + 1) \bmod 4) \mid v \in \mathbb{Z}_4 \}.
\]
Let $t_v \triangleq (v, v, (v + 1) \bmod 4)$ denote a transition parameterized by $v$.

As such, we extract the pseudolivelock relation consisting of a single cycle,
\[
H = \{ (t_v, t_{(v + 1) \bmod 4}) \mid v \in \mathbb{Z}_4 \}.
\]

Similarly, the propagation relation turns out to be a single cycle,
\[
E = \{ (t_v, t_{(v + 1) \bmod 4}) \mid v \in \mathbb{Z}_4 \}.
\]

The set $E$ is determined by the correspondence between the shadow (horizontal arc) connecting the target state of $t_v$ to the source state of $t_{(v + 1) \bmod 4}$, and transitions in the original transition set. In particular, any transition $t^{r-1}$ in the predecessor layer such that
\[
t^{r-1}.\mathtt{own} = t_v.\mathtt{pred}
\;\wedge\;
t^{r-1}.\mathtt{next} = t_{(v+1) \bmod 4}.\mathtt{pred}
\]
is an enabler of $t_{(v + 1) \bmod 4}$.

By construction, all horizontal arcs (shadows) correspond to actual transitions in the predecessor layer, ensuring closure of cycles in $P_r$. A sufficient condition for cycle closure is that the enablers of every closed walk (cycle) in $P_r$ themselves form a closed walk (cycle) of transitions (pseudolivelocks) in $P_{r-1}$ recursively enabled by its predecessor $P_{r-2}$ until the ring closes back to $P_r$ from $P_{r+1}$. This intuitively justifies the algorithmic decider in Section \ref{sec:algorithm}.
