...
\begin{enumerate}
  ...
  \item[\textbf{(F2)}] $E$ is defined on all of $\mathcal{L}$: every
        ...
        least one propagation arc. Here, $E$ is strenghthend by the equivariance conidtion with the predecessor's interface.
...
%========================================================
The preceding section introduced the livelock kernel
$\Lstar = \nu\Phi(T)$ and established that it is computable
in~$O(|T|^3)$ time: the Farahat witness is decidable.
We now show that~$\Lstar$ is semantically faithful:
$\Lstar \neq \emptyset$ if and only if a livelock execution
exists.
The argument proceeds through the execution semantics of
livelocks and yields, as a byproduct, an independent proof
of decidability via a bounded witness theorem.

\subsection{Livelock executions and periodic walks}

\begin{remark}[Periodic structure of livelocks]\label{rem:periodic}
If a livelock exists on a ring of size~$K$, it admits
a periodic execution in which each process repeatedly
traverses a closed walk in~$\Hstar(\Lstar)$.
...
fixed point of~$\Phi$, and that the witness consists of
simple cycles.
...
Let $C$ be a simple cycle of length~$N$ in~$\Hstar(\Lstar)$.
...
\begin{lemma}[Enabling walks are simple]
...
Each~$S_i$ is a valid simple cycle in~$\Hstar(\Lstar)$.
Since $\Lstar$ is a fixed point of~$\Phi$, every simple
cycle in~$\Hstar(\Lstar)$ admits enabling walks at every
process.
Therefore~$S_i$ propagates independently through
$P_{r-2}, P_{r-3}, \ldots$, producing a closed walk of
length~$|S_i|$ at each successive process.

The number of simple cycles of length~$|S_i|$
in~$\Hstar(\Lstar)$ is at most $\binom{|\Lstar|}{|S_i|}$.
...
whose transitions are drawn from~$\Lstar$.

In the livelock execution, $P_r$ executes~$C$.
By Remark~\ref{rem:periodic}, the enabling structure
...
A simple cycle admits no proper closed sub-walk:
any closed sub-walk of length less than~$N$ would revisit
a transition in fewer than~$N$ steps,
contradicting the simplicity of~$C$.
...
All such cycles are contained in~$\Lstar$.
In particular, $\Lstar \neq \emptyset$.
\end{lemma}

\begin{proof}
By Remark~\ref{rem:periodic}, a livelock admits periodic
equivariant closed walks in~$\Hstar(\Lstar)$.
...
The transitions of these simple cycles are sustained
by~$\Phi$, hence they belong to~$\Lstar$.
\end{proof}
...
\begin{lemma}[Simple cycles in $\Lstar$ construct a livelock]
\label{lem:construction}
If $\Lstar \neq \emptyset$, then there exists a simple cycle
in~$\Hstar(\Lstar)$ that supports a livelock on any ring
whose size~$K$ satisfies the circulation law.
\end{lemma}

\begin{proof}
Since $\Lstar = \nu\Phi(T)$ is a nonempty fixed point of~$\Phi$,
every transition in~$\Lstar$ lies on a cycle
in~$\Hstar(\Lstar)$.
...
\[
  ...
  \text{a livelock execution exists for some~$K$
  satisfying the circulation law.}
\]
\end{theorem}
...
If a livelock exists for a self-disabling protocol with
transition set~$T$ on state space~$\Zm$, then it admits
a witness consisting of a circulation of simple cycles
of length at most~$|T|$, independent of the ring size~$K$.
\end{theorem}

\begin{proof}
By Lemma~\ref{lem:reduction}, a livelock is supported by
simple cycles in~$\Hstar(\Lstar)$.
A simple cycle visits each transition at most once, so its
length is at most~$|\Lstar| \leq |T| \leq m^3$.
...
\begin{remark}
Lemma~\ref{lem:cl} (Farahat's Circulation Law) is not an independent
condition: given a Farahat witness $(\Hstar,\Estar,\Ls)$, a valid $K'$
is computable in finite time because $\Hstar$ is an SCC on the finite
set $\Ls$ (so $\ord(\Hstar)$ is finite) and $\Estar$ is a binary relation on the finite set $\Ls$ so repeated relational composition is eventually periodic.

The minimum admissible $K$ can be found by searching over pairs
$(|E|,K)$ with $(\Hstar)^{|E|}\cap E^K\neq\emptyset$ and $K\geq 2$,
using an $O(|\Ls|^2)$ matrix intersection check.

 Circulation is probably not independent once $\Ls$ is understood on the level of simple cycle support.Proving that requires an additional structural lemma about how $\Estar$ acts on the simple cycles of $\Hstar$. For thte scope of the current manuscript, we leave that to a subsequent follow-up.
\end{remark}
...
for parameterized livelock freedom and proivde a semi-algorithm for
...
for any $K$. The Circulation Law follows algebraically from three
properties of $\Ls$ (SCC, completeness of $\Estar$, finiteness).
