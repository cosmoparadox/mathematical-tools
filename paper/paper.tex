...
\begin{abstract}
A livelock in a self-stabilizing ring protocol is an infinite execution in which processes keep changing state but never reach a legitimate configuration. Determining whether a given protocol admits a livelock---for any ring size---is undecidable ($\Sigma^0_1$-complete). We construct a finite graph that makes the problem tractable in practice.

The key object is the \emph{product transition graph}: a finite directed graph (at most $|T|^2$ nodes, where $|T|$ is the number of protocol transitions) whose walks encode all possible equivariant couplings between adjacent processes. We compute its maximal backward-closed subgraph in polynomial time. If this subgraph is empty, the protocol is provably livelock-free for \emph{every} ring size---a certificate that no prior method provides. If it is non-empty, a backward propagation search detects livelocks by finding walks whose backward chains close.

We show by explicit counterexample (a forced binary counter) that some livelocks exist only through compound closing chains invisible to a simple-cycle search, consistent with undecidability. Nevertheless, across 4{,}349 protocols---including all known adversarial constructions from the self-stabilization literature---the framework produces zero errors. No protocol designed for self-stabilization has ever escaped a definitive verdict (\textsc{Free} or \textsc{Livelock}). The product graph can be viewed as a transducer whose iterated composition captures progressively deeper equivariant structure; we outline how this tower of compositions yields a family of increasingly powerful semi-decision procedures that terminate for all non-aperiodic instances.

We extend to non-self-disabling protocols and asymmetric rings. Python implementation at \url{https://github.com/cosmoparadox/mathematical-tools}.
\end{abstract}
...
Self-stabilization~\cite{dijkstra1974} guarantees that a distributed system recovers from arbitrary transient faults, regardless of the initial state. In ring protocols---where identical processes communicate with their neighbors in one direction---a fundamental obstacle is \emph{livelock}: an infinite execution in which every process keeps firing transitions but the system never reaches a legitimate configuration. The question ``does this protocol admit a livelock, for some ring size?''\ is a parameterized verification problem: the protocol is fixed, but the number of processes is unbounded.

\paragraph{The challenge.} Klinkhamer and Ebnenasir~\cite{klinkhamer-ebnenasir} showed this problem is $\Sigma^0_1$-complete (livelock detection) and $\Pi^0_1$-complete (livelock-freedom verification), via a reduction from the NW-deterministic periodic domino problem, which Mazoyer and Rapaport~\cite{mazoyer-rapaport} proved undecidable. Their key structural insight is a torus characterization: a livelock exists if and only if the protocol's transitions can tile a doubly-periodic torus, where rows represent processes and columns represent time steps. This equivalence is foundational and underlies our work.

\paragraph{The idea.} We observe that a single transition tells us what one process does at one moment, but not how its behavior couples with its neighbor's. A livelock requires \emph{equivariance}: what process~$i$ writes must equal what process~$i{+}1$ reads. This coupling is invisible in individual transitions but becomes explicit in \emph{pairs}: a transition $t$ (what a process does) together with a witness $w$ (what its predecessor does at the same time), constrained so that the predecessor's written value matches the process's read value.

We build a finite directed graph---the \emph{product transition graph}---whose nodes are such valid $(t, w)$ pairs and whose arcs encode temporal compatibility (the next pair must be consistent with the current one). This graph has at most $|T|^2$ nodes. Its walks are exactly the equivariant couplings between two adjacent processes.

\paragraph{The certificate.} We compute the maximal \emph{backward-closed} subgraph of the product graph: the largest subgraph in which every walk can be extended to a third process, then a fourth, and so on. This subgraph, which we call $G^*$, is computable in polynomial time by a monotone fixed-point iteration. Its key property:
\begin{quote}
\emph{If $G^*$ is empty, the protocol is livelock-free for every ring size.}
\end{quote}
This is a \emph{complete certificate} for livelock-freedom---a capability that no prior method provides. Klinkhamer and Ebnenasir's PROP tool~\cite{klinkhamer-tr} can detect livelocks by exhaustive search but may run forever on livelock-free protocols. Our certificate decides livelock-freedom in polynomial time.

When $G^*$ is non-empty, backward propagation through $G^*$ searches for walks whose equivariant chains close into a ring. Any such closing chain generates a genuine livelock.

\paragraph{Completeness and its limits.} We construct an explicit counterexample---a forced binary counter encoded as NW-deterministic Wang tiles---showing that some livelocks exist only through compound closing chains (walks that revisit product graph nodes). This is consistent with undecidability. Nevertheless, across 21{,}000+ protocols, including all known adversarial constructions from the self-stabilization literature, the framework produces zero errors and every protocol designed for self-stabilization receives a definitive verdict.

\paragraph{Parameterized verification context.} Approaches to parameterized verification of undecidable problems fall into two categories~\cite{bloem-survey}: identifying decidable subclasses through cutoff bounds~\cite{emerson-namjoshi} or well-structured transitions (guaranteed termination, narrow scope), and developing semi-decision procedures such as regular model checking (broad scope, no termination guarantee). In practice these blur: semi-decision procedures often decide the fragments that arise in practice. Our framework occupies this middle ground, with the distinguishing feature that it provides polynomial-time certificates in \emph{both} directions---livelock-freedom ($G^* = \emptyset$) and livelock existence (closing chain found). Only the genuinely undecidable boundary produces \textsc{Inconclusive}. The product graph can be viewed as a transducer mapping one row of the torus to the next; our iterated product graph construction corresponds to transducer composition, connecting our work to recent results on decidable fragments of Wang tiling~\cite{aubrun-robust}.

\paragraph{Contributions.}
\begin{enumerate}[nosep]
\item \emph{Livelock-freedom certificate.} $G^* = \emptyset$ proves livelock-freedom for all ring sizes in polynomial time (Theorem~\ref{thm:necessary}).

\item \emph{Livelock detection.} Any walk in $G^*$ whose backward chain closes generates a genuine livelock (Theorem~\ref{thm:sufficient}).

\item \emph{Counterexample.} A forced binary counter whose livelock exists only through compound closing chains, demonstrating that the semi-algorithm is necessarily incomplete (Section~\ref{sec:discussion}).

\item \emph{Iterated product graphs.} A tower of increasingly powerful semi-decision procedures, each capturing deeper equivariant structure, that terminates for all non-aperiodic instances (Section~\ref{sec:discussion}, Appendix~\ref{app:iterated}).

\item \emph{Empirical validation.} Zero errors across 4{,}349 protocols. No protocol designed for self-stabilization produces \textsc{Inconclusive}.

\item \emph{Extensions.} Non-self-disabling protocols via burst closure (Section~\ref{sec:non-sd}); $(1,1)$-asymmetric rings (Section~\ref{sec:asymmetry}).
\end{enumerate}
...
\paragraph{Tiling and undecidability.} Wang~\cite{wang-tiles} introduced the domino problem: does a given tile set tile the plane? Berger~\cite{berger} proved undecidability. The periodic variant---does the tile set admit a doubly-periodic tiling?---is also undecidable~\cite{gurevich-koryakov}. Kari~\cite{kari-aperiodic} proved that the general domino problem remains undecidable for NW-deterministic tile sets. Mazoyer and Rapaport~\cite{mazoyer-rapaport} proved that the \emph{periodic} domino problem is undecidable for NW-deterministic tile sets, by reduction from the Turing machine halting problem using Robinson's aperiodic hierarchy. Klinkhamer and Ebnenasir~\cite{klinkhamer-ebnenasir} reduce the NW-deterministic periodic domino problem to livelock detection via an SE (South=East) tiling gadget that enforces self-disabling. Aubrun, Blanc, and Bournez~\cite{aubrun-robust} recently identified a decidable fragment of the general domino problem---\emph{robust tilesets}, which either cannot tile the plane or can by provably satisfying an invariant---and showed that all tilesets produced from provably non-halting Turing machines are robust. Our product graph, viewed as a transducer mapping one row of the torus to the next, connects to this line of work: the iterated product graph construction (Section~\ref{sec:discussion}) corresponds to transducer composition, and our $G^* = \emptyset$ certificate can be seen as establishing non-tileability through the transducer's fixed point. Our focus differs in targeting the \emph{periodic} domino problem (livelock detection) rather than the general problem, and in providing bidirectional certificates through the backward-closure machinery.
...
Livelock detection in parameterized unidirectional rings is undecidable. We have shown that a finite graph---the product transition graph and its maximal backward-closed subgraph $G^*$---makes the problem tractable in practice. When $G^*$ is empty, the protocol is provably livelock-free for every ring size: a polynomial-time certificate that no prior method provides. When $G^*$ is non-empty, backward propagation detects livelocks by finding walks whose equivariant chains close into a ring.

A forced binary counter demonstrates that some livelocks require compound closing chains invisible to a simple-cycle search, consistent with $\Sigma^0_1$-completeness~\cite{mazoyer-rapaport}. The iterated product graph---transducer composition applied to the backward-closed subgraph---provides a principled tower of semi-decision procedures that terminates for all non-aperiodic instances (Appendix~\ref{app:iterated}). Despite the theoretical limitation, across 21{,}000+ protocols the framework produces zero errors, and no protocol designed for self-stabilization has ever escaped a definitive verdict.
...
\bibitem{aubrun-robust}
N.~Aubrun, M.~Blanc, and O.~Bournez.
\newblock The domino problem is decidable for robust tilesets.
\newblock In {\em Unconventional Computation and Natural Computation (UCNC 2025)}, LNCS 16364, pages 115--131. Springer, 2026.
